...
class CRSA:
    ...
    def __init__(self, meanings_A, meanings_B, categories, lexicon_S, prior, alpha=1.0, max_depth=None, tolerance=1e-6):
    ...
    def _sample_utterance(self, meaning_S, speaker_now):
        ...
        new_utt = utt_dist[utt_dist == utt_dist.max()].sample(n=1).index[0]
    ...
    def run_turn(self, speaker="A"):
        ...
        self.round_meaning_S = self.round_meaning_A if speaker == "A" else self.round_meaning_B
        lexicon, utterances = self.lexicon_S.create_from_past_utterances_and_meanings(self.past_utterances, speaker, meanings_S)
        prior = self.prior.copy() if speaker == "A" else self.prior.copy().transpose(1, 0, 2)
        ds = self.da if speaker == "A" else self.db
        dl = self.db if speaker == "A" else self.da
        ...
        # Update the dialog models
        self._update_dialog_models(speaker)
        
    
    def _update_dialog_models(self, speaker):
        
        ds_new = self.turns_history[-1].speaker.as_df.loc[:, self.past_utterances[-1]["utterance"]].values
        if speaker == "A" and self.da is None:
            self.da = ds_new.copy()
        elif speaker == "B" and self.db is None:
            self.db = ds_new.copy()
        elif speaker == "A":
            self.da *= ds_new
        else:
            self.db *= ds_new
        self.da[self.da <= ZERO] = ZERO
        self.db[self.db <= ZERO] = ZERO
